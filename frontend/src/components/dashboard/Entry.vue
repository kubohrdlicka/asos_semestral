<template>
  <div class="min-w-0">
    ...
    <div class="mt-1 flex items-center gap-x-2 text-xs text-gray-500">
      <p class="whitespace-nowrap">
        Due on
        <time :datetime="entry.deadline">{{ entry.deadline }}</time>
      </p>
      <svg viewBox="0 0 2 2" class="h-1 w-1 fill-current">
        <circle cx="1" cy="1" r="1" />
      </svg>
      <p class="truncate">Created by {{ entry.createdBy }}</p>
    </div>
  </div>
  <div class="flex flex-none items-center gap-x-4">
    ...
    >
      View project<span class="sr-only">, {{ entry.name }}</span>
    </a>
      ...
      >
        ...
        >
          ...
          <MenuItem v-slot="{ active }">
            ...
            >
              Move<span class="sr-only">, {{ entry.name }}</span>
            </a>
          </MenuItem>
          <MenuItem v-slot="{ active }">
            <a
